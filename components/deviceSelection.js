...
class DeviceSelection extends React.Component {
  ...
  render() {
    ...
    const deviceItems = (_.get(this.props, 'devices', [])).map(item => (
      <Option key={`select-${item.device_id}`} value={item.device_id} label={_.get(item, 'device_info.name', item.device_id)}>
        <List.Item>
          <List.Item.Meta
            ...
            description={`Last updated at ${moment(item.updatedAt).format("MMMM Do YYYY, h:mm:ss a")}`}
          />
        </List.Item>
      </Option>
    ));

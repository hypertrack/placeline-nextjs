...
class SummaryView extends React.Component {
  renderSegments(segments) {
    return segments.map((segment, i) => {
      <Timeline.Item key={`segment-${i}`}>{`${
        segment.type
      } activity for ${moment.duration(segment.duration, "s").humanize()} (${
        segment.distance
      } km)`}</Timeline.Item>;
    });
  }
        ...
        <div key={`summary-card-${i}`}>
          <Row gutter={4} type="flex" justify="space-around" align="middle">
            <Col span={8}>
              <Statistic
                title="Duration"
                value={moment
                  .duration(_.get(summary, "duration", 0), "s")
                  .humanize()}
              />
            </Col>
            <Col span={6}>
              <Statistic
                title="Distance"
                value={_.get(summary, "distance", 0) / 1000}
                suffix="km"
              />
            </Col>
            <Col span={6}>
          ...
          <Row gutter={8} type="flex" justify="space-around" align="middle">
            <Col span={16} style={timelineStyle}>
              <Timeline>
                <Timeline.Item color="green">{`Started on ${moment(
                  _.get(summary, "start_datetime", "")
                ).format("MMMM Do YYYY, h:mmA")}`}</Timeline.Item>
                {this.renderSegments(segments)}
                <Timeline.Item color="red">{`Ended on ${moment(
                  _.get(summary, "end_datetime", "")
                ).format("MMMM Do YYYY, h:mmA")}`}</Timeline.Item>
              </Timeline>
            </Col>
          </Row>
